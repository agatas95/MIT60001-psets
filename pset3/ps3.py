...
def update_hand(hand, word):
    ...
    for letter in str.lower(word):
        if letter not in new_hand and letter in VOWELS:
            new_hand['*'] -= 1
...
def is_valid_word(word, hand, word_list):
    ...
    for letter in word:
        if letter not in hand or modified_hand[letter] < 1:
            return False
        else:
            modified_hand[letter] -= 1
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while calculate_handlen(hand) > 0: # As long as there are still letters left in the hand:
        ...
        # Otherwise (the input is not two exclamation points):
            ...
            total += word_score
            print(f'"{word}" is worth {word_score} points. Total is {total} points.') # Tell the user how many points the word earned,
...
def substitute_hand(hand, letter):
    ...
    substitue[new_letter] = hand[letter]
    
    del(substitue[letter])
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    for i in range(num_hands):
        ...
        display_hand(hand)
        if can_substitute:
            response = input("Would you like to subsitute a letter? ")
        ...
        if can_replay:
            response = input("Would you to replay this hand? ")
            if str.lower(response) == 'yes' or str.lower(response) == 'y':
                hand_score = play_hand(hand, word_list)
